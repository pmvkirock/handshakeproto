...
class JobDes extends React.Component {
  ...
  applyJob = e => {
    ...
    if (
      this.props.getMajor == this.state.data[0].eligibility ||
      this.state.data[0].eligibility == 'All'
    ) {
      ...
      axios
        .post(
          'http://ec2-3-135-239-183.us-east-2.compute.amazonaws.com:8000/insertAppliEvents',
          data
        )
        .then(response => {
  ...
  getInfo = () => {
    ...
    if (this.props.idjob == '' || this.props.idjob == undefined) {
      ...
      axios
        .get(
          'http://ec2-3-135-239-183.us-east-2.compute.amazonaws.com:8000/getAllEvents'
        )
        .then(response => {
          if (response.status === 200) {
            this.setState({
      ...
      axios
        .get(
          'http://ec2-3-135-239-183.us-east-2.compute.amazonaws.com:8000/getEvents?idevents=' +
            this.props.idjob
        )
        .then(response => {
          if (response.status === 200) {
